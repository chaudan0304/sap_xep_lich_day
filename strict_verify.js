...
// 3. Timetable Grid: strictly ONLY content from Excel
const timetable = {};
classes.forEach(c => {
  timetable[c.id] = {
...
console.log(`Placed strictly ${placedCount} slots.`);

// 4. Check Teacher Conflicts
    ...
    classes.forEach(c => {
      ...
      if (slot && slot.teacherId && slot.subjectId !== 'CHAO_CO') {
        if (tMap[slot.teacherId]) {
          console.log(`CONFLICT for ${slot.teacherId} (${slot.teacherRaw}) at T${d} P${p}: ${tMap[slot.teacherId]} and ${c.id}`);
          teacherConflicts++;
        } else {
...
// 5. Check Room Conflicts
let roomConflicts = 0;
const specializedRooms = ['PHONG_TIN_HOC', 'PHONG_AM_NHAC', 'PHONG_MY_THUAT'];
for (let d = 2; d <= 6; d++) {
  for (let p = 1; p <= 7; p++) {
    specializedRooms.forEach(room => {
      ...
      classes.forEach(c => {
        ...
        if (slot && slot.roomId === room) {
          occ.push(`${c.id} (${slot.subjectId})`);
        }
      });
      if (occ.length > 1) {
        console.log(`Room CONFLICT in ${room} at T${d} P${p}: ${occ.join(', ')}`);
        roomConflicts++;
      }
    });
  }
}
console.log(`Specialized room conflicts: ${roomConflicts}`);

// 6. Teacher stats strictly from Excel
console.log('\n--- Teacher Scheduled Lessons (strictly from Excel) ---');
teachers.forEach(t => {
  let count = 0;
  classes.forEach(c => {
    for (let d = 2; d <= 6; d++) {
  ...
  if (count > 0 || t.assignedPeriods > 0) {
    console.log(`${t.id} (${t.name}): Scheduled = ${count} / Quota = ${t.dinhMuc}`);
  }
});
